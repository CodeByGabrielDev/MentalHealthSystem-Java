...
public class PacienteController implements PacienteDAO {
	...
	public Paciente callPacientes(int id) {
		...
		try (PreparedStatement ps = conexao.prepareStatement(call)) {
			...
			if (rs.next()) {
				...
				paciente.setStats(statusPaciente.fromDescricao(rs.getString("STATS")));
	...
	public List<Paciente> selectAll() {
		...
		try (PreparedStatement ps = conexao.prepareStatement(select)) {
			...
			while (rs.next()) {
				...
				/*
				 * ID_PESSOA int(11) NUMERO_PRONTUARIO int(11) ID_CONVENIO int(11) OBSERVACOES
				 * varchar(70)
				 */
	...
	public void insert(Paciente paciente) {
		...
		try (PreparedStatement psPessoa = conexao.prepareStatement(insertPessoa,
			...
			if (rs.next()) {
				...
				final String insertPaciente = "INSERT INTO PACIENTE(ID_PESSOA,NUMERO_PRONTUARIO,ID_CONVENIO,OBSERVACOES)"
						+ "VALUES(?,?,?,?)";
	...
	public void update(Paciente paciente) {
		// ADAPTAR BEGIN TRANSACTION !!!!!!!!!!!!
		// PARA NO SEGUNDO TRY DO UPDATE, CASO HOUVE ERRO OU EXECÇÃO NAQUELE UPDATE ELE
		// PODE DAR UM PS.ROLLBACK, ASSIM EVITADO CAMPOS INCONSISTENTES NO BANCO
		// UTILIZAR O SetAutoCommit(false);
		...
		/*
		 * NOME varchar(50) CPF varchar(100) RG varchar(60) DATA_NASCIMENTO date
		 * TELEFONE varchar(40) EMAIL varchar(40) ENDERECO varchar(40)
		 */
